...
def registro_facial():
    ...
    contra_info = contra.get() #Obtenemos la informacion almacenada en contra
    archivo = open(usuario_info, "w") #Abriremos la informacion en modo escritura
    archivo.write(usuario_info + ",")   #escribimos la info
    archivo.write(contra_info)
    archivo.close()
    print(contra_info)
    #Crear txt para el zip
    f=open(usuario_info+".txt","w")
    f.write("Secreto")
    f.close()
    #Compresion y del zip
    compression_level = 5 # 1-9
    pyminizip.compress(usuario_info+".txt", usuario_info, usuario_info + ".zip", contra_info, compression_level)
    #eliminar txt
    os.remove(usuario_info+".txt")
...
def registro():
    ...
    Label(pantalla1, text = "Contraseña del Zip").pack()  #Mostramos en la pantalla 1 la contraseña
...
def verificacion_login():
    ...
    if log_usuario in lista_archivos:   #Comparamos los archivos con el que nos interesa
        ...
        if log_contra in verificacion:
            print("Inicio de sesion exitoso, archivo descomprimido")
...
#------------------------------Vamos a capturar el rostro-----------------------------------------------------
    ...
    if usuario_login+".jpg" in im_archivos:   #Comparamos los archivos con el que nos interesa
        ...
        if similitud >= 0.98:
            ...
            lista_archivos = os.listdir()   #Vamos a importar la lista de archivos con la libreria os
            if usuario_login in lista_archivos:   #Comparamos los archivos con el que nos interesa
                archivo2 = open(usuario_login, "r")  #Abrimos el archivo en modo lectura
                for linea in archivo2.readlines():
                    columna=str(linea).split(",")
                with ZipFile(zips,"r") as z:
                    z.extractall("p_"+usuario_login,pwd=columna[1].encode())

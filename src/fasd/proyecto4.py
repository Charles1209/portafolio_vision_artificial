...
def insertar_persona(path_cropped_faces, faces, labels, names):
	...
	def guardar_rostros(path_cropped_faces, faces, labels, names, flag):
	...
	def reconocer_rostros(faces, labels, names):
		face_recognizer = cv2.face.LBPHFaceRecognizer_create()

		# Create a dictionary to map each unique name to a unique integer
		name_to_id = {name: idx for idx, name in enumerate(set(names))}

		# Convert the list of names to a list of corresponding integer labels
		labels = [name_to_id[name] for name in names]

		# Convert the list of labels to a numpy array of type int32 (required by OpenCV)
		labels = np.array(labels, dtype=np.int32)

		# Assuming 'faces' is a list of face data
		# faces = [...]

		print(labels)

		# Train the face recognizer with the faces and the integer labels
		face_recognizer = cv2.face.LBPHFaceRecognizer_create()
		face_recognizer.train(faces, labels)

		#face_recognizer.train(faces, np.array(nombre))

		test_img = cv2.imread("src/proyecto/Charles/1.png")
		predicted_img = predict(test_img, names)

		cv2.imshow("Persona", predicted_img)
		cv2.waitKey(0)
		cv2.destroyAllWindows()

	while flag == False:
		faces, labels, names, flag = guardar_rostros(path_cropped_faces, faces, labels, names, flag)
	
	reconocer_rostros(faces, labels, names)
...
def funcion_init():
	...
	path_faces = "src/proyecto/rostros.txt"
...
if __name__ == "__main__":
	path_cropped_faces, faces, labels, names = funcion_init()
